...
class Program
{
    ...
    {
        int[] vector = new int[] {1, 5, 8, 3, 6, 8, 9, 10};
        MyVector<int> newVector = new MyVector<int>(vector);

        newVector.AddAll(3, [99, 77, 33]);
        newVector.Print();

        int[]? vc = newVector.SubList(4,6);
        for(int i = 0; i < vc.Length; i++) Console.Write(vc[i] + " ");
        Console.WriteLine();

        Console.WriteLine(newVector.FirstElement());

        newVector.RemoveRange(1,5);
        newVector.Print();
    }
}
