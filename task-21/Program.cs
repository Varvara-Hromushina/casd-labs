...
class MyTreeMap<K, V> where K : IComparable<K> where V: IComparable<V>
{
    private class Node
    {
...
public class Program
{
    public static void Main(string[] args)
    {
        ...
        for(int i = 0; i< 5; i++) { list.Put(i, i * 6); }
        list.EntrySet();
        Console.WriteLine();

        Console.WriteLine("Дерево после удаления значения с ключом 2: ");
        list.Remove(2);
        list.EntrySet();
        Console.WriteLine();

        Console.WriteLine("Возврат последнего элемента без удаления: "+list.LastEntry());
        Console.WriteLine();

        Console.WriteLine("Возврат пары «ключ-значение», где ключ меньше заданного: ");
        list.LowerEntry(4);
    }
}