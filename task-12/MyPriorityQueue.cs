...
class MyPriorityQueue<T>  where T : IComparable<T>
{
    ...
    public MyPriorityQueue() { queue = new T[11]; size = 11; comparator = Comparer<T>.Default; }
    ...
    public void Heapify(int i)
    {
        ...
        while (true)
        {
            ...
            if(right < size && comparator.Compare(queue[right],queue[parents]) > 0)
                parents = right;

            if (left < size && comparator.Compare(queue[left],queue[parents]) > 0)
                parents = left;
